import os
import json

parent_log_full = r"C:\Users\Adfgd\.gemini\antigravity\brain\aef82a9c-45f4-4a59-92da-4de7bf8b6550\.system_generated\logs\transcript_full.jsonl"
output_path = r"C:\Users\Adfgd\.gemini\antigravity\scratch\cocacola-plantillas\initial_parent_steps.txt"

matches = []
if os.path.exists(parent_log_full):
    with open(parent_log_full, 'r', encoding='utf-8', errors='ignore') as f:
        for line_num, line in enumerate(f, 1):
            try:
                data = json.loads(line)
                step_index = data.get("step_index", 0)
                if step_index <= 30:
                    content = data.get("content", "")
                    tool_calls = data.get("tool_calls", [])
                    matches.append(f"=== STEP {step_index} ===")
                    if content:
                        matches.append(f"Content: {content[:1000]}...\n")
                    if tool_calls:
                        for tc in tool_calls:
                            matches.append(f"Tool call: {tc.get('name')}\nArgs: {json.dumps(tc.get('args'), indent=2)}\n")
            except Exception as e:
                pass

with open(output_path, 'w', encoding='utf-8') as out_f:
    out_f.write("\n".join(matches))

print(f"Saved initial steps to initial_parent_steps.txt")
